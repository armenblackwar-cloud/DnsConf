...
### 3) Exclusions (allowlist via env variable)
To avoid breaking required services (for example YouTube/Instagram/Telegram), set optional **environment variable** `EXCLUDE`.
...
+ Domains from `EXCLUDE` are skipped and will not be added to either `BLOCK` or `REDIRECT`.
+ Subdomains are also covered (for example, `api.telegram.org` is covered by `telegram.org`).
